...
void Shutdown()
{
    ...
    RenameThread("Performance-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Performance.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Performanced.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizePerformanceamount=<n> " + _("Keep N Performance anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Performance is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Performance is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Performance version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Performance server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Performance") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Performance to complete") << "\n";
    ...
    nAnonymizePerformanceAmount = GetArg("-anonymizePerformanceamount", 0);
    if(nAnonymizePerformanceAmount > 999999) nAnonymizePerformanceAmount = 999999;
    if(nAnonymizePerformanceAmount < 2) nAnonymizePerformanceAmount = 2;
    ...
    LogPrintf("Anonymize Performance Amount %d\n", nAnonymizePerformanceAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1PRFM+1000 == (.1PRFM+100)*10
       10PRFM+10000 == (1PRFM+1000)*10
    */
