...
int nAnonymizePerformanceAmount = 1000;
...
#else
    const char* pszModule = "Performance";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Performance
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Performance
    // Mac: ~/Library/Application Support/Performance
    // Unix: ~/.Performance
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Performance";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Performance.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Performanced.pid"));
