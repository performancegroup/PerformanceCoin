...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizePerformanceAmount"))
        settings.setValue("nAnonymizePerformanceAmount", 1000);
    nAnonymizePerformanceAmount = settings.value("nAnonymizePerformanceAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizePerformanceAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizePerformanceAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizePerformanceAmount:
            return QVariant(nAnonymizePerformanceAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizePerformanceAmount:
            nAnonymizePerformanceAmount = value.toInt();
            settings.setValue("nAnonymizePerformanceAmount", nAnonymizePerformanceAmount);
            emit AnonymizePerformanceAmountChanged(nAnonymizePerformanceAmount);
            break;
        default:
            break;
        }
