...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizePerformanceAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizePerformanceAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizePerformanceAmount = strAnonymizePerformanceAmount.remove(strAnonymizePerformanceAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizePerformanceAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizePerformanceAmount*COIN) nMaxToAnonymize = nAnonymizePerformanceAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizePerformanceAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizePerformanceAmount));
        strAnonymizePerformanceAmount = strAnonymizePerformanceAmount.remove(strAnonymizePerformanceAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizePerformanceAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizePerformanceAmount == 0){
