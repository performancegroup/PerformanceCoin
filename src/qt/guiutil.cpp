...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("Performance"))
        return false;
...
{
    // Convert Performance:// to Performance:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Performance://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "Performance:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Performance.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Performance.lnk
...
{
    return GetAutostartDir() / "Performance.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a Performance.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Performance\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Performance app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Performance app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Performance-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Performance-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("Performance-Qt"));
