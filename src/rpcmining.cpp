...
Value getsubsidy(const Array& params, bool fHelp)
{
    ...
    return (int64_t)GetProofOfStakeReward(pindexBest->nHeight, pindexBest->pprev, 0, 0);
}
...
Value getstakesubsidy(const Array& params, bool fHelp)
{
    ...
    return (uint64_t)GetProofOfStakeReward(pindexBest->nHeight, pindexBest->pprev, nCoinAge, 0);
}
...
Value getmininginfo(const Array& params, bool fHelp)
{
    ...
    obj.push_back(Pair("blockvalue",    (int64_t)GetProofOfStakeReward(pindexBest->nHeight, pindexBest->pprev, 0, 0)));
...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Performance is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Performance is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Performance is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Performance is downloading blocks...");
